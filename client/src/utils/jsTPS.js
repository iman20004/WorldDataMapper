...
export class UpdateLandmarks_Transaction extends jsTPS_Transaction {
    // opcodes: 0 - delete, 1 - add 
    constructor(regionId, newLand, opcode, addFunction, deleteFunction, index = -1) {
        super();
        this.regionId = regionId;
    ...
    async doTransaction() {
        let data;
        this.opcode === 0 ? { data } = await this.deleteFunction({ variables: { _id: this.regionId, landmarkToDelete: this.newLand } })
                          : { data } = await this.addFunction({ variables: { _id: this.regionId, newLandmark: this.newLand, index: this.index } });
        if (data) {
            console.log(data)
            return data;
        }
    ...
    async undoTransaction() {
        let data;
        this.opcode === 1 ? { data } = await this.deleteFunction({ variables: { _id: this.regionId, landmarkToDelete: this.newLand } })
                          : { data } = await this.addFunction({ variables: { _id: this.regionId, newLandmark: this.newLand, index: this.index } });
        if (data) {
            console.log(data)
            return data;
...
export class changeParent_Transaction extends jsTPS_Transaction {
    constructor(regionId, newParent, oldParent, callback, index = -1) {
        super();
        this.regionId = regionId;
    ...
    async doTransaction() {
        const { data } = await this.callback({ variables: { _id: this.regionId, index: this.index, newParentId: this.newParent } });
        if (data) {
            console.log(data)
            return data;
    ...
    async undoTransaction() {
        const { data } = await this.callback({ variables: { _id: this.regionId, index: this.index, newParentId: this.oldParent } });
        if (data) {
            console.log(data)
            return data;
